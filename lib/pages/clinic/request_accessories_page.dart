...
class _RequestAccessoriesPageState extends State<RequestAccessoriesPage> {
  ...
  submit_request() async {
    ...
    bool res = await Helpers.showConfirmation(
      context: context,
    ...
    if (res) {
      AccessoryRequestModel shop = AccessoryRequestModel(
        accessories: items,
        patient: widget.patient,
        doctor: AppData.get(context, listen: false).active_doctor,
      );

      // return print(shop.toJson());

      var dt = await ClinicDatabaseHelpers.add_update_accessory_request(
        context,
        data: shop.toJson(),
        showLoading: true,
        showToast: true,
