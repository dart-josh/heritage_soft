...
class AllPatientList extends StatefulWidget {
  ...
  @override
  State<AllPatientList> createState() => _AllPatientListState();
}
...
class _AllPatientListState extends State<AllPatientList> {
  ...
  List<PatientModel> filter_module() {
    ...
    return main_patients
        .where((element) => ((type_filter_on)
            ...
            : (mode_filter_on)
                ? filter_patients_by_mode(element, mode_filter_val)
                : true))
        .toList();
  }

  bool filter_patients_by_mode(PatientModel pm, String filter) {
    if (filter == 'Ongoing Assessment')
      return (pm.current_case_id != null &&
          pm.clinic_variables?.case_type == 'Assessment');
    else if (filter == 'Ongoing Treatment')
      return (pm.current_case_id != null);
    else if (filter == 'Awaiting Doctor')
      return (pm.current_case_id == null && pm.current_doctor != null);
  ...
  // main page
  Widget main_page() {
    main_patients = AppData.get(context).patients;
  ...
  List<String> type_values = [
    'Walk-In',
    for (var a in physio_hmo) a,
    'Clear',
  ];

  bool mode_expanded = false;
  bool mode_filter_on = false;
  String mode_filter_val = '';
                ...
                ? Wrap(
                    ...
                    children: [
                      ...
                      if (mode_filter_on)
                        Container(
                          decoration: BoxDecoration(
                            color: (mode_filter_val == 'Awaiting Doctor')
                                ? Color.fromARGB(255, 144, 103, 19)
                                : Color.fromARGB(255, 165, 19, 151),
                            borderRadius: BorderRadius.circular(25),
                          ),
                      ...
                      (element) => InkWell(
                        onTap: () {
                          if (element == 'Clear') {
                            setState(() {
                              mode_expanded = false;
                              mode_filter_on = false;
                              mode_filter_val = '';
                            });
                          } else {
                            setState(() {
                              mode_filter_val = element;
                              mode_filter_on = true;
                              filter_on = true;
                            });
                          }
