...
class _SessionPaymentDialogState extends State<SessionPaymentDialog> {
  ...
  calculate_sessions() {
    ...
    // using amount
      ...
      if (add_money_controller.text.isNotEmpty) {
        ...
        int sess = cost_p_session != 0 ? (total_amt / cost_p_session).truncate() : 0;
        new_session = sess;
  ...
  @override
  void initState() {
    cost_p_session = widget.session_details['cost_p_session'] ??  0;
    total_session = widget.session_details['total_session'] ?? 0;
    session_paid = widget.session_details['session_paid'] ?? 0;
    amount_paid = widget.session_details['amount_paid'] ?? 0;
    floating_amount = widget.session_details['floating_amount'] ?? 0;
