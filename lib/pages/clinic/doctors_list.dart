...
class _DoctorsListState extends State<DoctorsList> {
  dynamic get_doctors(dynamic data) async {
    DoctorModel doctor = DoctorModel.fromMap(data);

    AppData.set(context).update_doctor(doctor);
  }

  dynamic remove_doctor(dynamic id) async {
    AppData.set(context).delete_doctor(id);
  }

  initators() async {
    // get doctors
    await UserHelpers.get_doctors(context);
  }

  void initState() {
    doctor_profile_key = GlobalKey();
    initators();
    ServerHelpers.socket!.on('Doctor', (data) {
      get_doctors(data);
    });
    ServerHelpers.socket!.on('DoctorD', (data) {
      remove_doctor(data);
    });
  ...
  void dispose() {
    ...
    ServerHelpers.socket!.off('DoctorD');
    super.dispose();
  }
  ...
  // main box
  Widget main_box() {
    List<DoctorModel> doctors = AppData.get(context).doctors;
      ...
      onTap: () {
        ...
        // from homepage
          ...
          Future.delayed(
            ...
            () => doctor_profile_key.currentState!.openDrawer(),
          );
        }
      },
      child: Container(
              ...
              child: Center(
                child: Text(
                  not_active
                      ? 'Occupied'
                      : wating
                          ? '${total_patient} Patient${total_patient > 1 ? 's' : ''}'
                          : active
                              ? 'Available'
                              : 'Not Available',
                  style: TextStyle(
  ...
  Widget proceed_button() {
    ...
    int total_patient =
        active_doctor!.ong_patients.length + active_doctor!.pen_patients.length;

    bool active = active_doctor!.is_available && total_patient == 0;
    bool wating = active_doctor!.is_available && total_patient >= 1;
    bool not_active = active_doctor!.is_available && total_patient >= 5;
